...
if test "${fiovb.is_secondary_boot}" = "0"; then
	...
	if test "${fiovb.upgrade_available}" = "1"; then
		...
		if test ${bootcount} -gt ${bootlimit}; then
			...
			if test "${fiovb.bootupgrade_available}" = "1"; then
				...
				setenv image_path "${ostree_root}/usr/lib/firmware/imx-boot"
				echo "FIO: Writing ${image_path} ..."
				setenv start_blk 1042
				run load_image
				run update_image

				setenv image_path "${ostree_root}/usr/lib/firmware/u-boot.itb"
				echo "FIO: Writing ${image_path} ..."
				setenv start_blk 1300
				run load_image
				run update_image
	...
	if test "${fiovb.bootupgrade_available}" = "1" && test "${fiovb.upgrade_available}" = "0"; then
		echo "FIO: update primary boot path path with validated images ..."
